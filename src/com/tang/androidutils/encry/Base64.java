...
public class Base64 {
	private final byte[] encodingTable = { (byte) 'A', (byte) 'B', (byte) 'C',
	...
	public String encodeString(String data) {
		try {
			...
			MyLog.e("BASE64", "BASE64 encodeString 异常：" + e.getMessage());
		}
	...
	public String decodeString(String data) {
		try {
			return new String(decode(data.getBytes("UTF-8")), "UTF-8");
		}
		catch (UnsupportedEncodingException e) {
			MyLog.e("BASE64", "BASE64 decodeString异常：" + e.getMessage());
		}

		return null;
	}

	private byte[] discardNonBase64Bytes(byte[] data) {
	...
	private String discardNonBase64Chars(String data) {
	...
	private boolean isValidBase64Byte(byte b) {
		if (b == '=')
			return true;
		else if ((b < 0) || (b >= 128))
			return false;
